...
internal static class TypeComparer
{
    ...
    private static bool IsPredefinedTypeEquivalent(PredefinedTypeSyntax predefined, string identifier)
    {
        ...
        if (token.IsKind(SyntaxKind.StringKeyword)) return string.Equals(identifier, "String", StringComparison.Ordinal);
        if (token.IsKind(SyntaxKind.IntKeyword)) return string.Equals(identifier, "Int32", StringComparison.Ordinal);
        if (token.IsKind(SyntaxKind.BoolKeyword)) return string.Equals(identifier, "Boolean", StringComparison.Ordinal);
        if (token.IsKind(SyntaxKind.ObjectKeyword)) return string.Equals(identifier, "Object", StringComparison.Ordinal);
        if (token.IsKind(SyntaxKind.DecimalKeyword)) return string.Equals(identifier, "Decimal", StringComparison.Ordinal);
        if (token.IsKind(SyntaxKind.DoubleKeyword)) return string.Equals(identifier, "Double", StringComparison.Ordinal);
        if (token.IsKind(SyntaxKind.FloatKeyword)) return string.Equals(identifier, "Single", StringComparison.Ordinal);
        if (token.IsKind(SyntaxKind.LongKeyword)) return string.Equals(identifier, "Int64", StringComparison.Ordinal);
        if (token.IsKind(SyntaxKind.ShortKeyword)) return string.Equals(identifier, "Int16", StringComparison.Ordinal);
        if (token.IsKind(SyntaxKind.ByteKeyword)) return string.Equals(identifier, "Byte", StringComparison.Ordinal);
        return false;
    }
}