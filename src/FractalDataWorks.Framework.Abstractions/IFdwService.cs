...
/// to function properly. The configuration is provided during service initialization.
/// </remarks>
public interface IFdwService<in TConfiguration> : IFdwService
    where TConfiguration : FdwConfigurationBase
{
    /// <summary>
    /// Initializes the service with the provided configuration.
    /// </summary>
    /// <param name="configuration">The configuration object for this service.</param>
    /// <returns>
    ...
    /// <remarks>
    /// This method is called by the framework during service startup.
    /// Implementations should validate the configuration and prepare the service for use.
    /// If initialization fails, the service should not be considered available.
    /// </remarks>
