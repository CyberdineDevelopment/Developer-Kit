...
/// <summary>
/// Immutable class definition product created by ClassBuilder.
/// Represents a complete class declaration with all its members and metadata.
/// </summary>
public sealed class ClassDefinition : IAstNode, IMemberDefinition
{
    /// <summary>
    ...
    /// <summary>
    /// Gets the modifiers applied to this class.
    /// </summary>
    ...
    /// <summary>
    /// Gets the members of this class (methods, properties, fields, etc.).
    /// </summary>
    ...
    /// </summary>
    public bool IsSealed => Modifiers.HasModifier(Types.Modifiers.Sealed);

    /// <summary>
    /// Gets whether this class is partial.
    /// </summary>
    public bool IsPartial => Modifiers.HasModifier(Types.Modifiers.Partial);
    ...
    /// <summary>
    /// Internal constructor used by ClassBuilder.
    /// </summary>
    /// <param name="state">The builder state to create the definition from.</param>
    internal ClassDefinition(ClassBuilderState state)
    {
        Id = Guid.NewGuid();
        Name = state.Name;
        Parent = state.Parent;
        ...
        BaseTypes = state.BaseTypes.ToImmutableArray();
        GenericParameters = state.GenericParameters.ToImmutableArray();
        Members = state.Members.ToImmutableArray();
        ...
        // Create children list from members
...
internal sealed class ClassBuilderState
{
    ...
    public List<string> BaseTypes { get; set; } = new(StringComparer.Ordinal);
    public List<GenericParameterDefinition> GenericParameters { get; set; } = new();
    public List<IMemberDefinition> Members { get; set; } = new();
    ...
    public ClassBuilderState Clone()
    {
        return new ClassBuilderState
        {
            Name = Name,
            Parent = Parent,
            Access = Access,
            ...
            Members = new List<IMemberDefinition>(Members),
